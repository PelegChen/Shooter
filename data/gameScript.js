...
general_functions: {
    ...
    document.onkeydown = function(evt) {
        ...
        if (isEscape) {
         newLevel (true) // storeInLocal  ('save')
        }
        };
    ...
    function tShadow (borderline = 1, bordercolor = "black"){
        ...
        function borderLoop (line, offset){
            ...
            for (let i = 1; i <=4;i++){
                let s1 = 3%i ? '-': ''
                let s2 = i < 3 ? '': '-'
                txt1 += s1 +  (line - offset) + units + s2 +  line + units; txt1 += "0 " + bordercolor + ",";}
            return txt1;
        }
    ...
    function storeInLocal (command){
        ...
        // G.fileName = myFileName[0];

        switch (command){
            case 'check':
            let chk = localStorage.getItem(G.saveBooleanValue );
            let chkSession = sessionStorage.getItem(G.saveBooleanValue )
            if (chkSession == 'true'){G.isClickGameSaveInSessionStore = true} else {G.isClickGameSaveInSessionStore = true}
            if (chk == 'true' || chkSession == 'true'){return true} else {return false}
            break;

            case 'save':

            if (G.isClickGameSaveInSessionStore)  {
                sessionStorage.setItem(G.saveInLocalStorageKey, JSON.stringify(G.upgrade)); createEvent  ('save',G.saveInLocalStorageKey , JSON.stringify(G.upgrade));
                sessionStorage.setItem(G.saveBooleanValue, 'true'); createEvent  ('save',G.saveBooleanValue , 'true');

            break;}
            localStorage.setItem(G.saveInLocalStorageKey, JSON.stringify(G.upgrade)); createEvent  ('save',G.saveInLocalStorageKey , JSON.stringify(G.upgrade));
            localStorage.setItem(G.saveBooleanValue, 'true'); createEvent  ('save',G.saveBooleanValue , 'true');
            break;

            case 'load':
            var retrievedObject = localStorage.getItem(G.saveInLocalStorageKey);
            if (G.isClickGameSaveInSessionStore) {retrievedObject = sessionStorage.getItem(G.saveInLocalStorageKey) }
            G.upgrade = JSON.parse(retrievedObject);
            break;

            case 'reset':
            if (G.isClickGameSaveInSessionStore) {alert ('לא ניתן לאפס משחק, נא לאפס דרך ממשק קליק'); break}
            localStorage.setItem(G.saveBooleanValue, 'false')
            createEvent  ('reset',G.saveBooleanValue, 'false')
            break;
        }
    }
...
function buildMedia() {
    ...
    function buildsounds () {
         G.sound = {}
         G.sound.gunshot = new Audio('data/shotSound' + G.guntype + '.mp3');playSound(G.sound.gunshot ,true);
         G.sound.singlebullet = new Audio('data/singlebullet.mp3');playSound(G.sound.singlebullet ,true);
         G.sound.finished = new Audio('data/finished.mp3');playSound(G.sound.finished ,true);
         G.sound.twoguns = new Audio('data/twoguns.mp3');playSound(G.sound.twoguns ,true);
         G.sound.timeup = new Audio('data/timeup.mp3');playSound(G.sound.timeup ,true);
         G.sound.beep = new Audio('data/beep.mp3');playSound(G.sound.beep ,true);
         G.sound.hitsound = new Audio('data/hitsound.mp3');playSound(G.sound.hitsound ,true);
         G.sound.buzzer = new Audio('data/buzzer.mp3');playSound(G.sound.buzzer ,true);
         G.sound.gunload = new Audio('data/gunload.mp3');playSound(G.sound.gunload ,true);
         G.sound.upgrade = new Audio('data/upgrade.mp3');playSound(G.sound.upgrade ,true);
         G.sound.endLevel = new Audio('data/endLevel.mp3');playSound(G.sound.endLevel ,true);
         G.sound.voice = [];
    ...
    function buildGun(Isgun2 = false) {
        ...
        if (Isgun2){theId = "2"; guncontainer_sideposition = "20%";direction = -1; guncontainer_left = "-10%"; guncontainer_right = "70%"; gunposition = 100- G.gunrightLimi; }
    ...
    function buildHud () {
        ...
        stl (points, {
            ...
            "background" : "linear-gradient(90deg, red 0%, transparent , transparent, transparent, transparent, transparent, transparent, transparent, transparent)", "border": "0.5vh solid black","borderColor":'black', "border-radius": "15vh",
        ...
        Id ("plaque").appendChild(hud); hud.id = "hud";
        pointstxt = document.createElement("text"); pointstxt.id = "pointstxt";
        pointstxt.innerHTML = G.upgrade.playerPoints;
        let ranktxt = "דרגה " + G.upgrade.playerRank + " ";
        ...
        stagetxt = document.createElement("text"); stagetxt.id = "stagetxt";
        ...
        for (let i = 1; i <= G.playerBullets; i++){
            ...
            let Onebullet = document.createElement("img"); Onebullet.src = "data/bullet.png";
            stl (Onebullet, {
                'position': 'absolute',
                'top':'0%',
    ...
    var pagecontainer = document.createElement("div"); pagecontainer.id = "pagecontainer";
    stl(pagebackground, {
        'position': 'fixed',
        'height': "100%",
        'width': "100%",
    ...
    var galleryContainer = document.createElement("div"); galleryContainer.id = "galleryContainer";
    stl(pagecontainer, {
        'position': 'absolute',
        'height':'100%',
        'width':'99%',
    ...
    let leftpole = document.createElement("div"); leftpole.id = "leftpole"
    let rightpole = document.createElement("div"); rightpole.id = "rightpole"
    let leftcurtain = document.createElement("div"); leftcurtain.id = "leftcurtain"
    let rightcurtain = document.createElement("div"); rightcurtain.id = "rightcurtain"
    let plaque = document.createElement("div"); plaque.id = "plaque"
    let bottomgallery = document.createElement("div"); bottomgallery.id = "bottomgallery"
    let lightsign = document.createElement("div"); lightsign.id = "lightsign"
    let botoom_of_sign = document.createElement("div");botoom_of_sign.id = "botoom_of_sign";
    ...
    var wood = document.createElement("div");wood.id = "wood"
    var targetscontainer = document.createElement("div"); targetscontainer.id = "targetscontainer"
    ...
    let buttonsUI = document.createElement('div'); buttonsUI.id ='buttonsUI';
    stl  (buttonsUI, {
        'position':'absolute',
        'top':'8%',
        'right': '3%',
        ...
        'width': '30%',
        'scr' : '',
        "zIndex":'4'}); ; Id ("plaque").appendChild(buttonsUI);
    let volumeButton = document.createElement('img'); volumeButton.src = "data/volumeon.png";
    ...
    let pauseButton = document.createElement('img'); pauseButton.src = "data/pause.png";
    ...
    let timer = document.createElement('div'); timer.id = "timer";
    stl  (timer, {
        'position':'absolute',
        'top':'53%',
        'height' : '22%',
...
function togglePause (upgraded = false, finished = false){
    ...
    function blacker (gry = 0,blr = 0,brt = 100,dir = -1){
        ...
        let pb = Id('pageBody');
        if (dir == -1) { gry < 90 ? gry++: "";blr < 4 ? blr++ : "";brt > 50 ? brt--: "" };
        if (dir == 1) { gry > 0 ? gry--: "";blr > 0 ? blr-- : "" ;brt < 100 ? brt++: "" };
...
function moveCurtain (action = 'open'){
    ...
    const leftOpen = -62 ; const leftClose = -10 // left closed 30
    const rightOpen = 90 ; const rightClose = 45 // left open -15
    let advance = 1;
    ...
    rightcurtainX = Id ('rightcurtain').style.left;
        let dir = 1; if (action == 'open') { dir = -1}
        var timer1 = setInterval (()=>{
            moving (dir)
        }, 15)
        setTimeout (()=>{ G.curtainMoving = false;
            clearInterval(timer1);},1000);
...
function gunHidesOrComesUp (direction = -1,gunNumber = 1,directAction = "") {
    ...
    function lowerGun (finalHieght){
        ...
        setTimeout(()=> {y = y + (direction * momentum) // (direction * VerticalGunmove)
            gun.style.bottom = y + "%";
...
function visuaGamelLoader (isForsave = false) {
    function GameloadingAnimation (status = "draw",precent = 0, isStartOfGame = false){
        if (status == "draw"){
            ...
            var loadContainer = document.createElement("div");loadContainer.id = "loadContainer";
            galleryContainer.appendChild(loadContainer)
            var loadContainerText = document.createElement("p");loadContainerText.id = "loadContainerText";
            ...
            stl (loadContainer, {"position":"fixed",'height': "5%",
            'width': "50%","left":"25%" , "top":   tp + "%","background" : "linear-gradient(90deg, red 0%, transparent , transparent, transparent, transparent, transparent, transparent, transparent, transparent)", "zIndex":"13", "border": "5px solid black","borderColor":bordercolor, "border-radius": "15px",
            "background-clip": "content-box",
            "padding":"0.3%", "backgroundSize" : "100%"})
...
function gunFiring (event) {
    function gunRecoil (gunNumber = 1){
        ...
        function addedRecoil (Styleproperty,distance = gunRecoiljumpXY ) {
            ...
            Styleproperty += distance; return Styleproperty + "%"}
    ...
    if (G.isGun2 && G.isOneHandLoading && (G.isGunloading[2] || G.isGunloading[1])){
        ...
        if (G.lastFiredGun == 2){G.lastFiredGun = 1}
        else if (G.lastFiredGun == 1){G.lastFiredGun = 2}
    }
    ...
    switch (theIdOftar.id) {
            ...
            case 'rightpole':
            break;
            case 'jump2banks': menu ('targetbank');return; break;
            case 'jump2save': menu ('saving'); return; break
            case 'jump2upgrad': menu ('upgrades'); return; break
            case 'back2game':  togglePause ();return; break;
            case 'back2menu': menu ('back2settings'); return; break;
            default:
    ...
    function fadeHoles (Thehole){
        ...
        setTimeout(function(){
            ...
            if (!check){return};
            let parent = Thehole.parentElement;parent.removeChild(Thehole);},timeOfshowingShot * 2);
    }
 ...
 // drawing a hole
    let x = event.clientX;
    let y = event.clientY;
    let hole = document.createElement("img"); hole.id = "hole"
...
function gunLoading(gunNumber = 1,canPfire = false) {
    ...
    if(gunNumber == 1) { if (G.isGunloading[1] == true){return}; G.isGunloading[1] = true;}
    if(gunNumber == 2) { if (G.isGunloading[2] == true){return}; G.isGunloading[2] = true;}
...
function stopWatch (countdown,stopTheCount){ // starts the stoper
    ...
    if (stopTheCount) {G.stopWatchTIME = 0; clearInterval(G.stopWatch);
        G.minutes = 0; G.seconds = 0; G.miliseconds = 0 ;   timePrint ();
        return
       }
...
function updatehud (dontCheckTheScore = false){
    ...
    function newUpgradesNotice(newToken)  {
      ...
      if (!Id("newUpgrades")) {
        ...
        newUpgrades.innerHTML = "עלית בדרגה ! שיפורים חדשים זמינים"
    ...
    let delay = 80;
    if (G.playerBullets == G.upgrade.maxBullets && (G.lastBulletStatus != G.playerBullets)
){ }else {delay = 0 }
    G.lastBulletStatus = G.playerBullets;
...
function addTarget (x,y,direction){
    ...
    let target = {}; target.className = "onBoard"
    let safty = 0;
...
function moveTargets (speed = 15,hieght = 20,direction = -1) {
    ...
    let mytimer = setInterval(function(){ //the timer that moves the target
        ...
        if (left < -10 || left > 102 || isoffboard) { // kills the target
            ...
            if (G.isPlaying) {moveTargets (speed,hieght,direction)
...
function shootSomeTarget(event){
    ...
    function hit (idOftarget) {
        ...
        if (G.upgrade.loadUpgrade[3] && ((G.playerBullets + 2) < G.upgrade.maxBullets) && (G.playerBullets > 0) && (rand > 65)){G.playerBullets +=2 ; setTimeout (()=>{
            showPointsAfterHit("+2" , 'gun','white',120, {'fontSize': '8vmax',
            'fontSize': '8vmin'})
            playSound(G.sound.singlebullet)},300)}
        showPointsAfterHit("+" + pointsAdded,"target" + idOftarget,'Chartreuse');

        if ((G.isGun2 == false) && (G.upgrade.twoGunsUpgrade[1]) && (G.hitStreak >= G.upgrade.HitstreakFor2guns)) {G.isGun2 = true; playSound(G.sound.twoguns) }
        updatehud ()
        stl (Id("target" + idOftarget),{
             'opacity': '0'
          //"display":"none"
        ...
        if (G.gameType.draw && G.isPlaying){
            ...
            for (i in child){
             ...
             ;
            setTimeout(()=>{ tarcon.style.opacity = '1'; clearTargets();builddrawlevel()},700);
            return}
        setWord();
...
function setWord(){
    ...
    if (G.gameType.static == true){
        ...
        while (true){
            ...
            if (breaking > 100){G.IsclearBoard = true; clearTargets(); break}
...
function showPointsAfterHit (ptext, targetid,color,speed = 40, styleObject) {
    ...
    let svg = document.createElement('div'); svg.id = "show points"
    stl(svg,{
    ...
    let time2 = setTimeout(()=>{if (svg == null){return} ; StylelFader (svg,10);},speed - 38)
    let timer1 = setInterval (()=>{
        ...
        if (Pre2Num (svg.style.bottom) - 20 >  startposition ){
            ...
            let tc = Id('targetscontainer')
            if (Is(svg) && tc.contains(svg)) {tc.removeChild(svg)
        };}
        },speed)
...
function builddrawlevel() {
    G.gameType.static = true;
    function timedstart (){
        gunHidesOrComesUp (1,1,'up'); if (G.isGun2){gunHidesOrComesUp (1,2,'up')}
    ...
    for (i = 3; i >= 0; i--){
        ...
        setTimeout(()=>{
            if (t == 0) {
                if (G.ispause){setTimeout(()=>{clearTargets();builddrawlevel()},700); return}
                timedstart ()}
            else {
...
function menu (type='settings'){
    ...
    function upgrade (mission,numOfUpgrade) {
        ...
        let upGrd = false;
        if (mission == 'upgradeNum') { upGrd = true; playSound(G.sound.upgrade)}
        switch (numOfUpgrade) {
            case 11: UpgrdStatus = G.upgrade.loadUpgrade[1]
            if (upGrd) {G.upgrade.loadUpgrade[1] = true ;  G.gunloadingTime = G.gunloadingTime * G.gunloadingChange}
            canItUpgrade = true;
            ;break;
            case 21: UpgrdStatus = G.upgrade.loadUpgrade[2]
            if (upGrd) {G.upgrade.loadUpgrade[2] = true ; G.gunloadingTime = G.gunloadingTime * G.gunloadingChange}
            canItUpgrade = G.upgrade.loadUpgrade[1];
            ;break;
            case 31: UpgrdStatus = G.upgrade.loadUpgrade[3]
            if (upGrd) {G.upgrade.loadUpgrade[3] = true}
            canItUpgrade = G.upgrade.loadUpgrade[2];
            ;break;
            case 12: UpgrdStatus = G.upgrade.bulletsUpgrade[1]
            if (upGrd) {G.upgrade.bulletsUpgrade[1] = true; G.upgrade.maxBullets +=2; G.playerBullets = G.upgrade.maxBullets}
            canItUpgrade = true;
            ;break;
            case 22: UpgrdStatus = G.upgrade.bulletsUpgrade[2]
            if (upGrd) {G.upgrade.bulletsUpgrade[2] = true; G.upgrade.maxBullets +=2; G.playerBullets = G.upgrade.maxBullets}
            canItUpgrade = G.upgrade.bulletsUpgrade[1]
            ;break;
            case 13: UpgrdStatus = G.upgrade.hitUpgrade[1]
            if (upGrd) { G.upgrade.hitUpgrade[1] = true}
            canItUpgrade = true;
            ;break;
            case 23: UpgrdStatus = G.upgrade.hitUpgrade[2]
            if (upGrd) { G.upgrade.hitUpgrade[2] = true}
            canItUpgrade =  G.upgrade.hitUpgrade[1];
            ;break;
            case 32: UpgrdStatus = G.upgrade.twoGunsUpgrade[1]
            if (upGrd) {G.upgrade.twoGunsUpgrade[1] = true;G.upgrade.maxBullets +=1; G.playerBullets = G.upgrade.maxBullets}
            canItUpgrade = (G.upgrade.hitUpgrade[2] || G.upgrade.bulletsUpgrade[2])
            ;break;
            case 41: UpgrdStatus = G.upgrade.twoGunsUpgrade[2]
            if (upGrd) {G.upgrade.twoGunsUpgrade[2] = true} // two hand loading
            canItUpgrade = G.upgrade.twoGunsUpgrade[1]
            ;break;
            case 42: UpgrdStatus = G.upgrade.twoGunsUpgrade[3]
    ...
    function drawRankupSign (){
        ...
        function writeLightsign(){
            ...
            let myText = 'עלית בדרגה ויש לך'
            let noimprove = "השיפורים הקיימים מופיעים בצבע:"
            myText += "&nbsp&nbsp" + G.upgrade.tokens + "&nbsp&nbsp";
            myText += 'שיפורים זמינים.';
            myText += '';
        ...
        for (x = 1; x <= 4; x++){
            for (y = 3; y >= 1; y--){
                let morex = 3;  let moresize = 0;
                if ( x >= 4){ morex = 4; moresize = 3 }
                ...
                if (G.upgrade.tokens > 0 && !upgrade ('isUpgradetrue',bnsNum) &&upgrade ('canItUpgrade',bnsNum))
                {animateAvupgrades (bnsNum)};
    ...
    function drawSettingsSign () {
        ...
        let instructions = document.createElement('div'); instructions.id = 'instructions';
        let jump2upgrad = document.createElement('div'); jump2upgrad.id = 'jump2upgrad';
        let jump2banks = document.createElement('div');jump2banks.id = 'jump2banks';
        let jump2save = document.createElement('div'); jump2save.id = 'jump2save'



        stl(jump2save,txtstyle, {'position': 'absolute',
            'right': '45%',
            'top': "40%",
            'height': (13 + "%"),
            'width': (13 +  "%"),
            ...
            'backgroundSize':'40%',
            'textAlign': 'center',})
        stl(jump2banks, {'position': 'absolute',
            'right': '27%',
            'top': "40%",
            'height': (13 + "%"),
            'width': (13 +  "%"),
            ...
            "backgroundSize": "100%",}, txtstyle);
        stl(jump2upgrad, {'position': 'absolute',
            'right': '70%',
            'top': "40%",
            'height': (13 + "%"),
            'width': (13 +  "%"),
            ...
            'background': 'url("data/gun' + G.guntype +  '.png")',
            "backgroundSize": "100%",}, txtstyle);
        stl(instructions, {'position': 'absolute',
            'right': '25%',
            'top': "22%",
            'height': (13 + "%"),
            'width': (60 +  "%"),
        ...
        let targetTxt = "בנק מטרות";
        let upgTxt = "שיפורים";
        let insTxt = "&nbsp"+ "המטרה במשחק לעבור את השלבים, על ידי ירי על התמונות הנכונות. חלק מהשלבים מוגבלים בזמן. אחרי צבירת נקודות, אפשר להרוויח דרגות, ולזכות בשיפורים.";
        if (isFinished) {
            insTxt = '<font color="yellow">' +  "  🏆🏆 סיימת את המשחק ! 🏆🏆 " + "<br>";
            insTxt += "ניתן להתחיל משחק חדש על ידי : שמירה למחשב זה -> התחלת משחק חדש."
            insTxt += "<br>" + "אפשר גם להמשיך לשחק אך השלבים יהיו קשים יותר !"
        ...
        jump2save.innerHTML = "שמירה למחשב זה"
    ...
    function drawSaveSign () {
        function submitF(formArray) {
            if (G.isClickGameSaveInSessionStore) { Formtext.innerHTML = '<span style="color:red;">' +  'מחובר דרך האתר.  יש להתנתק דרך האתר או לאפס את המשחק דרך הממשק.' + '</span>';
            return;
        }
            if (formArray === 'clear'){
            ...
            if (input.length < 2){Formtext.innerHTML = 'שם צריך להכיל לפחות 2 אותיות' ; return}
        ...
        let input = document.createElement("input"); input.id ='input';
        ...
        let txt = "ניתן לשמור את ההתקדמות על המחשב הזה"


        if (storeInLocal ('check') || G.isClickGameSaveInSessionStore){

        txt = G.upgrade.nameOfplayer + ', ';
        txt += ' ההתקדמות שלך במשחק נשמרת'
        if (G.isClickGameSaveInSessionStore) { txt += ' באתר ' }
        }
        if (type == 'iAmNot'){
            txt = 'אם את/ה לא ' + '&nbsp'
            txt += G.upgrade.nameOfplayer + '&nbsp'
            txt += 'ניתן ללחוץ על התחלת משחק חדש'
            if (G.isClickGameSaveInSessionStore) { txt = '<span style="color:red;">' ;txt += ' מחובר דרך האתר. יש להתנתק מהאתר ולהתחיל משחק חדש. ' + '</span>'}
        }
        Formtext.innerHTML = txt


        submit.setAttribute('type',"submit");
        submit.setAttribute('value',"שמירה");
        clear.setAttribute('type',"button");
        clear.setAttribute('value',  "התחלת משחק חדש");
        ...
        stl (input, styleObj); stl (submit,styleObj) ; stl (clear,styleObj)
    ...
    function drawReurnSingns (type = ''){
        ...
        if (type !== 'back2game') {
            let wood = Id("woodsign");
            let back2menu = document.createElement('div');back2menu.id = 'back2menu';
            stl(back2menu, {'position': 'absolute',
                'right': '36%',
                'top': "56%",
                'overflow': 'hidden',
                'height': (4 + "%"),
    ...
    function woodSign (op = 0){
        let gallery_sideposition = "10%"
        let woodsign = document.createElement("div"); woodsign.id = 'woodsign'
        let woodsigncontainer = document.createElement("div"); woodsigncontainer.id = 'woodsigncontainer'
        stl(woodsign, {
            'position': 'fixed',
            "bottom" : "-20vh",
...
function newLevel (finish = false,timeup = false) {
    ...
    function clear (id0) {
        ...
        setTimeout (()=>{
            ...
            if (!check){return};
            obj.parentNode.removeChild(obj)}, 200)
    }
    function endLevel (){
        ...
        if (!timeup) {playSound(G.sound.endLevel)} else {stopWatch (0,true); playSound(G.sound.timeup) ; G.upgrade.timesFailed++ }
        if (Is(Id('endbutton'))) {var endbutton = Id('endbutton'); var endbuttonBg = Id('endbuttonBg')} else {
            var endbutton = document.createElement ('div'); endbutton.id = 'endbutton';
            var endbuttonBg = document.createElement ('div'); endbuttonBg.id = 'endbuttonBg';
        }
        ...
        let finishText ="&nbsp" +   'סיימת את שלב' + "&nbsp&nbsp&nbsp&nbsp"+ G.upgrade.playerLevel + " &nbsp" + "!"

        if (timeup){finishText = 'נגמר הזמן'}
        ...
        setTimeout (()=>{clear ('endbutton'); clear ('endbuttonBg'); moveCurtain('close');
        if (!timeup) {G.upgrade.playerLevel++ ;G.upgrade.timesFailed = 0 ;} ;
        if (storeInLocal ('check')) {storeInLocal ('save');}
        updatehud (true);
        setTimeout (()=>{ clearTargets (); curtainScreen() ;  stopWatch (0,true);},800); gunHidesOrComesUp (-1,1); if (G.isGun2){gunHidesOrComesUp (-1,2)}  },3000)
    ...
    function curtainScreen(){
        ...
        if (try1){gamesign = try1}else {
            ...
            gamesign = new Image; gamesign.id = "gamesign";
        ...
        if (Id('stageinfo') ) {var stageinfo = Id('stageinfo'); var startbutton = Id('startbutton')
        var startbuttonBg = Id('startbuttonBg')} else {
        var stageinfo = document.createElement ('div'); stageinfo.id = 'stageinfo'
        var startbutton = document.createElement ('div'); startbutton.id = 'startbutton';
        var startbuttonBg = document.createElement ('div'); startbuttonBg.id = 'startbuttonBg';
        }
        let areYouHim = Elm ('areYouHim');

        areYouHim.innerHTML ='<br>'+ 'אני לא   ' + G.upgrade.nameOfplayer ;
        ...
        switch (Lvltyp) {
            ...
            extraInstructions =  'נסו לירות על המטרות לפי המילה שמופיעה בשלט למעלה';
            break;
            case 2:
            extraInstructions = 'שליפה מהירה!' + '<br>'
            case 3:
            extraInstructions += ' עליכם לפגוע ב ' + '&nbsp' + needToHitPerStage +  '&nbsp' + 'מטרות לפני שנגמר הזמן ';
            break;
            case 4: extraInstructions = ' עליכם לפגוע בכל המטרות בסדר הנכון '
            break;
            case 5: extraInstructions = 'עליכם לפגוע ' +  '&nbsp' + needToHitPerStage + '&nbsp' + ' פגיעות נכונות ברצף '
            break;
        }
        ...
        let instructions = '';
        if (G.upgrade.nameOfplayer){instructions += G.upgrade.nameOfplayer + " , " + "אלו ה"}
        instructions +=  "הוראות:" + '<br>'
        instructions += extraInstructions;
        //instructions += 'בהצלחה !.'
        stageinfo.innerHTML = instructions;
        startbutton.innerHTML = "התחל שלב" + "  &nbsp &nbsp"+ G.upgrade.playerLevel
        ...
        StylelFader ( stageinfo,50,true) ; StylelFader ( startbutton,50,true);StylelFader (startbuttonBg ,50,true)
        flasher()
        startbutton.addEventListener('click', ()=>{initiateLevels()})
        areYouHim.addEventListener('click', ()=>{menu ('iAmNot'); G.ispause = true;
        pauseButton.src = "data/pause.png";})
        if ((G.isGun2 == true) && (G.upgrade.twoGunsUpgrade[1]) && (!G.upgrade.twoGunsUpgrade[3])) {G.isGun2 = false;  gunHidesOrComesUp(-1,2);}
    ...
    function initiateLevels(justVars = false) {
        if (!justVars){
        clear ('stageinfo'); clear ('startbutton');clear ('startbuttonBg'); clear ('areYouHim'); let gs = Id('gamesign');if (gs) {gs.remove();};

         setTimeout(function(){moveCurtain('open');},1)
        // general values for all levels
            }
        G.targetsHit = 0; G.hitStreak = 0;
        if (!justVars){ G.isPlaying = true};
        G.gameType.moving = false; G.gameType.countdown = false;
        G.gameType.draw = false;
        G.gameType.static = false; G.gameType.hitWords = false;
        ...
        let t;
        switch (Lvltype){case 1: t = 12; break; case 2: t = 5; break;
             case 3: t = 15;break; case 4: t = 18; break; case 5: t = 10} // 5 15 18 10
             t += LvlChallange;
...
G.upgrade.playerLevel = 1
if(storeInLocal ('check')){storeInLocal ('load') } else if (G.isClickGameSaveInSessionStore) {
    storeInLocal ('save')
