...
general_functions: {
  ...
  function expand(obj, finS = 1.5, speed = 20) {
    ...
    function adder(obj) {
      if (obj.className == "downScaling") {
        setTimeout(() => adder(obj), 300);
        return;
      }
  ...
  function shrink(obj, finS = 1, speed = 20) {
    ...
    function diminisher(obj) {
      ...
      t -= p;
      if (t >= finsize) {
        setTimeout(() => diminisher(obj), speed);
      } else {
    ...
    switch (type) {
      case "text":
        obj = {
          ...
          fontWeight: "bolder",
          textShadow:
            "0.1vw 0.5vh 1px black,-1px -1px 1px black,-1px 1px 1px black, 1px -1px 1px black, 9px 8px 0px rgba(0,0,0,0.15)",
        };
        break;
      case 1:
        obj = {
  ...
  function XptoRank(stage = G.upgrade.playerRank - 1) {
    let pointsTonextStage = Math.round(
      Math.pow(G.XpExponential, stage) * G.XpFirstRank
    );
    if (pointsTonextStage < G.XpFirstRank) {
      return 0;
    }
  ...
  function sinusOne(num) {
    // gets sinus function results from 0 to 1
    ...
    switch (command) {
      case "save":
        localStorage.setItem(
          G.saveInLocalStorageKey,
          JSON.stringify(G.upgrade)
        );
        createEvent("save", G.saveInLocalStorageKey, JSON.stringify(G.upgrade));
        break;
      case "check":
        if (G.upgrade.nameOfplayer) {
          return true;
        } else {
          return false;
        }
      ...
      case "reset":
        if (G.isTheGameConnectedToClick) {
          alert(G.TXT.cantResteGameDoWithClicl);
          break;
        }
...
function setTXT() {
  if (G.EN) {
    G.TXT = {
      ...
      cantResteGameDoWithClicl:
        " Can't reset, please reset through the site interface ",
      rank: " Rank ",
      ...
      upgrades: " Upgrades ",
      theGoalOfTheGameIs:
        " Your goal is to finish all the levels by shooting at the right objects. Some of the levels have a time limit. After gaining points, you can advance in rank and win upgrade options. ",
      savingToThisPC: " Save to this PC ",
      youFinishedTheGame: "   🏆🏆 You've finished the game! 🏆🏆  ",
      youCanStartAnewGame:
        " You can start a new game by: save to this pc -> start a new game. ",
      youCanContinueButItsHard:
        " You can also continue playing but the levels would be harder ",
      connectedThroughSiteYouMustLogout:
        " Connected through the site. Please reset using the site interface. ",
      nameMustHave2chars: " Name has to  ",
      fromNowtourProgWillBeSaved:
        " From now on, your game progress would be saved. ",
      saveWasNotFoundRefreshThewindow:
        " No saved game was found. You can start a new game by refreshing the tab. ",
      doYouWantToDeleteAndStartNew:
        " Are you sure you want to delete all progress and start a new game? ",
      theseAre: " These are ",
  ...
  G.TXT = {
    ...
    timeIsUp: "נגמר הזמן",
    IAmNot: "אני לא   ",
    tryToShootTargetsByWords:
      "נסו לירות על המטרות לפי המילה שמופיעה בשלט למעלה",
    ...
    upgrades: "שיפורים",
    theGoalOfTheGameIs:
      "המטרה במשחק לעבור את השלבים, על ידי ירי על התמונות הנכונות. חלק מהשלבים מוגבלים בזמן. אחרי צבירת נקודות, אפשר להרוויח דרגות, ולזכות בשיפורים.",
    savingToThisPC: "שמירה למחשב זה",
    youFinishedTheGame: "  🏆🏆 סיימת את המשחק ! 🏆🏆 ",
    youCanStartAnewGame:
      "ניתן להתחיל משחק חדש על ידי : שמירה למחשב זה -> התחלת משחק חדש.",
    youCanContinueButItsHard: "אפשר גם להמשיך לשחק אך השלבים יהיו קשים יותר !",
    connectedThroughSiteYouMustLogout:
      "מחובר דרך האתר.  יש להתנתק דרך האתר או לאפס את המשחק דרך הממשק.",
    nameMustHave2chars: "שם צריך להכיל לפחות 2 אותיות",
    fromNowtourProgWillBeSaved: ", מעכשיו ההתקדמות שלך במשחק תישמר",
    saveWasNotFoundRefreshThewindow:
      "לא נמצא מידע שמור. ניתן להתחיל משחק חדש על ידי רענון החלון.",
    doYouWantToDeleteAndStartNew:
      "האם אתם מעוניינים למחוק את כל ההתקדמות ולהתחיל משחק חדש ?",
    theseAre: "אלו ה",
...
function buildMedia() {
  function gunMove(event) {
    ...
    function moving(gunNumber) {
      ...
      if (gunNumber == 2) {
        elem.style.left = shoulBeleft + leftalign + "%";
      }
  ...
  function bulildwords() {
    ...
    for (var x = 1; x <= G.maxnumberOfSigns; x++) {
      ...
      for (var i = 1; i <= G.maxtargePerWord; i++) {
        ...
        G.word[x].target[i].addEventListener(
          "mousedown",
          shootSomeTarget,
          false
        );
  ...
  function buildHud() {
    ...
    stl(points, {
      ...
      background:
        "linear-gradient(90deg, red 0%, transparent , transparent, transparent, transparent, transparent, transparent, transparent, transparent)",
    ...
    let rankName =
      G.ranksArry[G.upgrade.playerRank] || G.ranksArry[G.ranksArry.length - 1];
    ranktxt += rankName;
    ...
    stl(pointstxt, {
      ...
      fontSize: G.Fontsiz1 + "vmin",
      textShadow:
        "0.5vw 0.5vh 1px black,-1px -1px 1px black,-1px 1px 1px black, 1px -1px 1px black, 9px 8px 0px rgba(0,0,0,0.15)",
    });
  ...
  document.getElementById("pageBody").addEventListener(
    "dragstart",
    function () {
      return false;
    },
...
function togglePause(upgraded = false, finished = false) {
  function blacker(gry = 0, blr = 0, brt = 100, dir = -1) {
    if (dir == 1 && G.ispause == true) {
      return;
    }
    if (dir == -1 && G.ispause == false) {
      return;
    }
    ...
    let styleobject = {
      filter:
        "grayscale(" + gry + "%) blur(" + blr + "px)  brightness(" + brt + "%)",
    };
    stl(gc, styleobject);
    stl(pb, styleobject);
    setTimeout(() => {
...
function moveCurtain(action = "open") {
  ...
  function moving(direction) {
    ...
    if (action == "open") {
      if (xleft > leftOpen) {
        leftcurtain.style.left = xleft + "%";
      }
      if (xright < rightOpen) {
        rightcurtain.style.left = xright + "%";
      }
    } else if (action == "close") {
      if (xleft < leftClose) {
        leftcurtain.style.left = xleft + "%";
      }
      if (xright > rightClose) {
        rightcurtain.style.left = xright + "%";
      }
    }
    xleft += adding;
...
function checkLoadstatus() {
  ...
  let precentOfLoaded =
    ((numberoferrors + numberOfloads) * 100) /
    (G.maxnumberOfSigns * (G.maxtargePerWord + 1));
  G.numOffoundSigns = numberOfsigns;
  G.EstimatedtargetGroups = Math.ceil(
    (numberOfloads - numberOfsigns) / numberOfsigns
  );
  return Math.round(precentOfLoaded);
}

function visuaGamelLoader(isForsave = false) {
  function GameloadingAnimation(
    status = "draw",
    precent = 0,
    isStartOfGame = false
  ) {
...
function gunFiring(event) {
  function gunRecoil(gunNumber = 1) {
    ...
    for (x = 1; x < speedOFmovment; x++) {
      ...
      if (x > speedOFmovment / 2) {
        themovement = themovement * -1;
      }
      setTimeout(function () {
  ...
  if (event.button !== 0 || (!G.isPlaying && !G.ispause)) {
    return;
  }
  if (G.isGunloading[1] && !G.isGun2) {
    return;
  }
  ...
  if (
    G.isGun2 &&
    G.isOneHandLoading &&
    (G.isGunloading[2] || G.isGunloading[1])
  ) {
    if (G.isGunloading[2]) {
      G.lastFiredGun = 1;
    } else if (G.isGunloading[1]) {
      G.lastFiredGun = 2;
    }
  } else if (G.isGun2) {
    if (G.lastFiredGun == 2) {
      G.lastFiredGun = 1;
    } else if (G.lastFiredGun == 1) {
      G.lastFiredGun = 2;
    }
  }
  ...
  if (
    theIdOftar.id == "volumeButton" ||
    theIdOftar.id == "pauseButton" ||
    G.ispause ||
    theIdOftar.id == "newUpgrades"
  ) {
    return;
  }
  ...
  function fadeHoles(Thehole) {
    ...
    setTimeout(function () {
      StylelFader(Thehole, timeOffade);
    }, timeOfshowingShot);
    setTimeout(function () {
  ...
  if (G.playerBullets <= 0) {
    ...
    G.isGun2 &&
    G.isOneHandLoading &&
    G.playerBullets < G.upgrade.maxBullets / 2
  ) {
    if (!G.isGunloading[1] && !G.isGunloading[2])
...
function gunLoading(gunNumber = 1, canPfire = false) {
  ...
  function blockFireWhileLoading() {
    if (
      G.isGun2 &&
      (G.isGunloading[1] == false || G.isGunloading[2] == false)
    ) {
      stl(Id("galleryContainer"), {
        cursor: "url('data/crosshair.png'), auto",
      });
      return;
    }
    ...
    cursor > 2 ? (cursor = 1) : 0;
    stl(Id("galleryContainer"), {
      cursor: "url('data/loadcursor" + cursor + ".png'), auto",
    });
    setTimeout(() => {
      //    stl(Id('galleryContainer'), {'cursor': "url('data/loadcursor"  + cursor + ".png'), auto"});
      if (!G.isGun2 || (G.isGunloading[1] && G.isGunloading[2])) {
  ...
  if (gunNumber == 1) {
    if (G.isGunloading[1] == true) {
      return;
    }
    G.isGunloading[1] = true;
  }
  if (gunNumber == 2) {
    if (G.isGunloading[2] == true) {
      return;
    }
    G.isGunloading[2] = true;
  }
  setTimeout(() => {
    gunHidesOrComesUp(-1, gunNumber);
  }, 300);
  setTimeout(() => {
    gunHidesOrComesUp(1, gunNumber);
  }, G.gunloadingTime - 300);
  setTimeout(() => {
    EndGunloading(gunNumber);
  }, G.gunloadingTime);
  blockFireWhileLoading();
  playSound(G.sound.gunload);
}

function stopWatch(countdown, stopTheCount) {
  // starts the stoper
  G.stopWatchTIME = 0;
  let TimeClicks = 0;
  if (countdown) {
    TimeClicks = countdown * 10;
  }
  ...
  function timePrint() {
    ...
    let TheTimeText =
      formattedminutes + ":" + formattedseconse + "." + G.miliseconds;
    if (G.isPlaying === false) {
      timertxt.innerHTML = TheTimeText;
      return;
    }
    if (countdown && TimeClicks > 0) {
      TimeClicks--;
    } else if (!countdown) {
      TimeClicks++;
    }
    G.seconds =
      Math.floor((TimeClicks / ChangeTimeInterval) * 10) - G.minutes * 60;
    ...
    G.minutes = Math.floor(
      Math.floor((TimeClicks / ChangeTimeInterval) * 10) / 60
    );

    if (countdown && G.seconds < 10 && G.minutes == 0) {
      theColor = "yellow";
    }
    if (
      countdown &&
  ...
  G.stopWatch = setInterval(function () {
    timePrint();
  }, ChangeTimeInterval);
}
...
function updatehud(dontCheckTheScore = false) {
  ...
  function newUpgradesNotice(newToken) {
    ...
    function sizer(dir = 1) {
      ...
      if (dir == 1 && scale < maxSize) {
        setTimeout(() => {
          sizer(dir);
        }, sizeTim);
      } else if (dir == 1 && scale >= maxSize) {
        setTimeout(() => {
          sizer(-1);
        }, sizeTim);
  ...
  if (!G.upgrade.tokens && Id("newUpgrades")) {
    Id("hud").removeChild(Id("newUpgrades"));
  }
  if (G.upgrade.tokens) {
    newUpgradesNotice(newTok);
  }
  ...
  for (let i = 1; i <= G.upgrade.maxBullets; i++) {
    ...
    if (G.playerBullets < i) {
      stl(Onebullet, { opacity: "0.2" });
    }
  ...
  let botoomXp =
    G.upgrade.playerRank == 1 ? 0 : XptoRank(G.upgrade.playerRank - 2);
  ...
  if (dontCheckTheScore) {
    return;
  }
  if (!G.gameType.static || G.gameType.draw) {
    checkScore();
  } else if (G.gameType.static) {
...
function moveTargets(speed = 15, hieght = 20, direction = -1) {
  ...
  function isLastTargtNear() {
    ...
    for (let i in children) {
      if (children[i].style && children[i].className !== "gunhole") {
        ...
        if (
          Math.abs(startleft - left) < nearx &&
          Math.abs(bottom - hieght) < neary
        ) {
          let name = Math.round(
            Number(children[i].id.replace("target", "")) / G.TargetIdMutiplayer
          );
          return true;
        }
      }
  ...
  let mytimer = setInterval(function () {
    //the timer that moves the target
    if (G.ispause == true) {
      return;
    }
    ...
    if (left < -10 || left > 102 || isoffboard) {
      // kills the target
...
function shootSomeTarget(event) {
  function missed(idOftarget) {
    ...
    if (G.PenaltyPerTarget && !G.upgrade.hitUpgrade[2]) {
      ...
      if (G.upgrade.playerPoints <= 0) {
        G.upgrade.playerPoints = 0;
      } else if (G.upgrade.playerPoints <= XptoRank(G.upgrade.playerRank - 2)) {
        G.upgrade.playerPoints = XptoRank(G.upgrade.playerRank - 2);
      }
      updatehud();
    }
  ...
  function hit(idOftarget) {
    ...
    let isAugmented = 1;
    if (G.upgrade.hitUpgrade[1]) {
      isAugmented = G.upgrade.pointsAugmentation;
    }
    ...
    if (
      G.upgrade.loadUpgrade[3] &&
      G.playerBullets + 2 < G.upgrade.maxBullets &&
      G.playerBullets > 0 &&
      rand > 65
    ) {
      G.playerBullets += 2;
    ...
    if (
      G.isGun2 == false &&
      G.upgrade.twoGunsUpgrade[1] &&
      G.hitStreak >= G.upgrade.HitstreakFor2guns
    ) {
      G.isGun2 = true;
...
function setWord() {
  ...
  function IsWordOnboard(numOfword) {
    ...
    for (let i in children) {
      if (children[i].style && children[i].className !== "gunhole") {
        ...
        tar = id0.replace(/\D/g, "");
        tar = Math.round(tar / G.TargetIdMutiplayer);
        if (numOfword == tar && children[i].style.opacity == 1) {
          return true;
        }
      }
    }
  ...
  setTimeout(() => {
    playSound(G.sound.voice[rndSign]);
  }, 400);
}
...
function showPointsAfterHit(ptext, targetid, color, speed = 40, styleObject) {
  ...
  stl(txt, {
    ...
    height: "20%",
    textShadow:
      "0.5vw 0.5vh 1px black,-2px -2px 1px black,-2px 2px 1px black, 2px -2px 1px black, 9px 8px 0px rgba(0,0,0,0.15)",
  });
  svg.appendChild(txt);
  ...
  let timer1 = setInterval(() => {
    ...
    if (Pre2Num(svg.style.bottom) - 5 > startposition) {
...
function buildMovementLevel(NumOfLevel) {
  ...
  for (r = 1; r <= rows; r++) {
    ...
    if (G.onlyOneDirection) {
      rndDirec = 1;
    }
    for (t = 1; t <= tarPerRow; t++) {
      G.targettimers[i] = setTimeout(function () {
        moveTargets(speed - rndspeed, h, rndDirec);
      }, t * gapBetweenTargets);
      i++;
    }
  }
...
function builddrawlevel() {
  ...
  function timedstart() {
    ...
    if (G.isGun2) {
      gunHidesOrComesUp(1, 2, "up");
    }
    for (i = 1; i <= 3; i++) {
      addTarget(i * 25 - 5, 55, 1);
    }
    setWord("static");
  }
  ...
  function upgrade(mission, numOfUpgrade) {
    ...
    switch (numOfUpgrade) {
      ...
      case 41:
        ...
        if (upGrd) {
          G.upgrade.twoGunsUpgrade[2] = true;
        } // two hand loading
      ...
      case 42:
        ...
        if (upGrd) {
          G.upgrade.twoGunsUpgrade[3] = true;
        } // always 2
        canItUpgrade = G.upgrade.twoGunsUpgrade[1];
        break;
    }
    ...
    if (G.upgrade.twoGunsUpgrade[2]) {
      G.isOneHandLoading = true;
    }
    if (G.upgrade.twoGunsUpgrade[3]) {
      G.isGun2 = true;
    }
    if (mission == "isUpgradetrue") {
      return UpgrdStatus;
    }
    if (mission == "canItUpgrade") {
      return canItUpgrade;
    }
    if (mission == "clear") {
      for (i = 1; i < 4; i++) {
  ...
  function drawRankupSign() {
    ...
    for (x = 1; x <= 4; x++) {
      for (y = 3; y >= 1; y--) {
        ...
        if (
          G.upgrade.tokens > 0 &&
          !upgrade("isUpgradetrue", bnsNum) &&
          upgrade("canItUpgrade", bnsNum)
        ) {
    ...
    function submitF(formArray) {
      if (G.isTheGameConnectedToClick) {
        Formtext.innerHTML =
          ...
          G.TXT.connectedThroughSiteYouMustLogout +
          "</span>";
        return;
      }
      if (formArray === "clear") {
      ...
      if (input.length < 2) {
        Formtext.innerHTML = G.TXT.nameMustHave2chars;
        return;
      }
  ...
  function drawTargetIndex() {
    ...
    for (y = 1; y <= lines; y++) {
      ...
      for (Prei = 1; Prei <= targetPerLine; Prei++) {
        if (y == lines && Prei > G.numOffoundSigns % targetPerLine) {
          continue;
        }
...
function checkScore() {
  ...
  if (
    G.gameType.moving &&
    !G.gameType.streak &&
    G.targetsHit >= G.targetsNeedHit
  ) {
    newLevel(true);
    return;
  }
  if (G.gameType.draw && G.targetsHit >= G.targetsNeedHit) {
    newLevel(true);
    return;
  }
  if (G.gameType.static && G.targetsHit >= G.targetsNeedHit) {
    newLevel(true);
    return;
  }
  if (G.gameType.streak && G.hitStreak >= G.targetsNeedHit) {
    newLevel(true);
    return;
  }
}
...
function newLevel(finish = false, timeup = false) {
  ...
  function endLevel() {
    ...
    if (Is(Id("endbutton"))) {
      var endbutton = Id("endbutton");
      var endbuttonBg = Id("endbuttonBg");
    } else {
  ...
  function curtainScreen() {
    ...
    stl(startbuttonBg, iStyle("text"), {
      ...
      zIndex: "10",
      textShadow:
        "-12px 8px 40px rgba(254,254,254,1), 9px 8px 40px rgba(254,254,254,1), 9px -10px 40px rgba(254,254,254,1)",
      opacity: "0",
    });
    let instructions = "";

    if (G.upgrade.nameOfplayer) {
      instructions += G.upgrade.nameOfplayer + " , " + G.TXT.theseAre;
    }
    ...
    startbutton.innerHTML =
      G.TXT.beginLevel + "  &nbsp &nbsp" + G.upgrade.playerLevel;
    ...
    if (storeInLocal("check")) {
      Id("galleryContainer").appendChild(areYouHim);
    }
    ...
    if (
      G.upgrade.playerLevel === G.finalLevel ||
      G.upgrade.playerLevel === G.finalLevel + 1
    ) {
      setTimeout(() => {
...
setTimeout(function () {
  checkLoadstatus();
}, 1000); //need this to get the randomizer to work
