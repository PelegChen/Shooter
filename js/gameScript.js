...
function menu (type='settings'){
    ...
    function drawSaveSign () {
        function submitF(formArray) {
            if (G.isClickGameSaveInLocalStore) { Formtext.innerHTML = '<span style="color:red;">' +  'מחובר דרך האתר.  יש להתנתק דרך האתר או לאפס את המשחק דרך הממשק.' + '</span>';
            return;
        }
            if (formArray === 'clear'){
        ...
        if (G.isClickGameSaveInLocalStore) { txt += ' באתר ' }
        }
        if (type == 'iAmNot'){
            txt = 'אם את/ה לא ' + '&nbsp'
            txt += G.upgrade.nameOfplayer + '&nbsp'
            txt += 'ניתן ללחוץ על התחלת משחק חדש'
            if (G.isClickGameSaveInLocalStore) { txt = '<span style="color:red;">' ;txt += ' מחובר דרך האתר. יש להתנתק מהאתר ולהתחיל משחק חדש. ' + '</span>'}
        }
